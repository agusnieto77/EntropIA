...
`turbo.json` MUST define pipelines for `build`, `dev`, `lint`, `typecheck`, and `test`. The `build` pipeline MUST depend on `^build` (topological). The `dev` pipeline MUST be `persistent: true` and uncached.
