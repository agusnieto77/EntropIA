# Delta for FTS5 Full-Text Search
...
#### Scenario: Phrase search returns exact phrase matches

- GIVEN an item with extracted text "acta del cabildo de Buenos Aires"
- WHEN `searchByText('"cabildo de Buenos Aires"')` is called (pre-sanitized)
- THEN that item appears in results
- AND items without the exact phrase do not appear

#### Scenario: Multi-token query matches all tokens

- GIVEN items indexed: item A has "cabildo Buenos Aires", item B has "carta al gobernador"
- WHEN `searchByText('cabildo gobernador')` is called
- THEN item A and item B do NOT match (neither has both tokens)
- AND an item with both tokens would match
